...
if uploaded_file:
    try:
        ...
        with zipfile.ZipFile(zip_buffer, "a", zipfile.ZIP_DEFLATED, False) as zip_file:
            zip_file.writestr("ARIE_MCDM_Result.xlsx", excel_bytes.getvalue())
            zip_file.writestr("ARIE_Score_Plot.png", img_bytes.getvalue())

        st.pyplot(fig)

        st.download_button(
            label="📥 Download All (Excel + Chart)",
            data=zip_buffer.getvalue(),
            file_name="ARIE_MCDM_Results.zip",
